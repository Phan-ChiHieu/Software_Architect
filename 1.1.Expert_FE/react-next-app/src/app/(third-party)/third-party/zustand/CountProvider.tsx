...
export default function CountProvider({ initialCount, children }: CountProviderProps) {
  //
  const [store] = useState(() =>
    createStore<CountStore>((set) => ({ count: initialCount, inc: () => set((state) => ({ count: state.count + 1 })) }))
  );
